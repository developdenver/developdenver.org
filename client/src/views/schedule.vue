	...
	computed: {
		...
		filteredEvents() {
			return this.events
				...
				.filter(event =>
					this.filterByVenue(this.selectedVenue)(
		...
		thursdayEventsByTime() {
			return this.filteredEvents.filter(
				event => event.properties.day === 'Thursday',
			);
		},
		fridayEventsByTime() {
			return this.filteredEvents.filter(
				event => event.properties.day === 'Friday',
			);
		},
		...
		fridayEventsByVenue() {
			return this.splitByVenues(
				this.events.filter(event => event.properties.day === 'Friday'),
			);
		},
	},
	methods: {
		sortEvents(firstEvent, secondEvent) {
			return this.compareNumbers(
				firstEvent.properties.startTime,
				secondEvent.properties.startTime,
			);
		},
		...
		filterByCategory(selectedCategory) {
			return eventCategory =>
				this.categories
					...
					.includedCategories.includes(eventCategory);
		},
		filterByVenue(selectedVenue) {
			return eventVenue =>
				this.venues
					.find(venue => venue.id === selectedVenue)
					.includedVenues.includes(eventVenue);
		},
			...
			return [
				{
					...
					events: events.filter(
						event => event.properties.venue === "Bigsby's Folly",
					),
				},
				{
					label: 'Catalyst',
					events: events.filter(
						event => event.properties.venue === 'Catalyst',
					),
				},
				{
					label: 'Big Trouble',
					events: events.filter(
						event => event.properties.venue === 'Big Trouble',
					),
				},
				{
					label: 'Source Hotel',
					events: events.filter(
						event => event.properties.venue === 'Source Hotel',
					),
				},
				{
					label: 'Urban Cyclist',
					events: events.filter(
						event => event.properties.venue === 'Urban Cyclist',
					),
				},
			];
		},
